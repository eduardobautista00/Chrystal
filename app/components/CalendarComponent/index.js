    ...
    const fetchProperties = async () => {
      try {
        ...
        const userProperties = propertyArray.filter(
          property => property.user_id === loggedInUserId && property.status === 'available'
        );
        ...
        renderItem={({ item }) => (
          <View style={styles.todoItem}>
            ...
            <Text>
              ...
              <Text style={{ fontWeight: 'bold', color: item.status === 'done' ? 'green' : item.status === 'pending' ? 'orange' : 'black' }}>
            ...
            <View style={styles.actions}>
              <TouchableOpacity 
                style={[styles.doneButton, item.status === 'done' && styles.disabledDoneButton]} 
                onPress={() => handleMarkAsDone(item.id)} 
                disabled={item.status === 'done'} // Disable if status is done
              >
                <Text style={styles.actionButtonText}>Done</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.editButton, item.status === 'done' && styles.disabledEditButton]} 
              ...
              <TouchableOpacity 
                style={[styles.deleteButton, item.status === 'done' && styles.disabledDeleteButton]} 
...
const fetchBuyerDataByEmail = async (email) => {
  if (debounceTimeout) {
  ...
  const newTimeout = setTimeout(async () => {
    try {
      const response = await fetch(`${apiUrl}/get-buyer-by-email?email=${encodeURIComponent(email)}`);
      if (!response.ok) {
      ...
      setBuyerData(data);
      //console.log('Buyer Data:', data.buyer_id);
    } catch (error) {
      console.error('Error fetching buyer data:', error);
      Alert.alert('Error', 'An error occurred while fetching buyer data');
    }
            ...
            {modalTab === 'seller' ? (
              ...
              <>
                <ScrollView style={{ height: 200, marginBottom: 10}}>
                  ...
                  {/* Show the email field regardless of buyer existence */}
                  <TextInput
                    placeholder="Buyer Email"
                    value={buyerEmail}
                    onChangeText={(text) => {
                      setBuyerEmail(text);
                      fetchBuyerDataByEmail(text); // Call the fetch function with debounce
                    }}
                    style={styles.input}
                  />

                  {buyerAlreadyExists ? (
                    // Additional fields are not shown if buyer already exists
                    null
                  ) : (
                    // Show all buyer fields if buyer does not exist
                    <>
                      <TextInput
